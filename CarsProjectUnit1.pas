   ...
   Begin
    If Exists then
     Begin
      ...
      If ((xCent>bot.xCent-bot.collisionSize div 2) and (xCent<bot.xCent+bot.collisionSize div 2)
       and (yCent>bot.yCent-bot.collisionSize div 2) and (yCent<bot.yCent+bot.collisionSize div 2))
       then
        Begin
         ArrBullets[IndexMax].Exists:=False;
         Dec(BulletsExists);
         If Exists then Dec(bot.HitPoints,10);
        end;
     end;
   end;
 ...
 If  not (collisionBool)  then
  Begin
   ...
   bot.yCent:=bot.yCent+bot.MultiplierDirectionY;
                 ...
                 //!!!!!!!
{!!!!} If bot.HitPoints<=0 then ShowMessage('Dead');   //!!!!! тут бот дохнет
                 //!!!!!!!
//
